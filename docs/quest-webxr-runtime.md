...
The tabletop preserves its heading while the viewer moves. Only holding the table position handle enables facing updates; release freezes the resulting heading and recenter resets it. Actions panes open above their invoking hotbar button with a fixed bottom pivot and pitch chosen so the panel center faces the viewer. Pause and its submenus use ordinary centered placement, clearing the Actions anchor and its dragged offset. Immersive Extended uses six 90px button columns (570px including gaps), fits the modal to that grid, and centers horizontally over the hotbar while retaining the invoking button height and bottom hinge. Layout changes are immediate, and Customize remains visible in immersive Extended only; flat/mobile Extended keeps it hidden. These sizing and animation overrides are scoped to `nh3d-webxr-active` and do not affect flat presentation.

The Quest APK puts VR first in Options and selects it whenever Options opens. The flatten-items default is off; explicit saved choices are retained. Native immersive FPS draws actions first, then a depth-tested, alpha-tested mask replay for loot on the authoritative player tile only, then the remaining HUD and dialogs. Controllers and held weapons retain the last foreground pass. Loot uses alpha=.75 and the fourth corner-tag value=.75 to distinguish the extra layer from older controller-only frames. The existing UI heartbeat carries a validated two-bit `X-NH3D-Loot-Hits` header; only the covered actions pane yields input to an opaque loot hit on the player tile, while dialogs and existing UI captures retain priority. No CPU framebuffer readback or additional full-resolution render target is introduced. The device shader harness covers both eye rectangles/orientations, loot/action and modal ordering, controllers, MR alpha and legacy frames. Full headset interaction/performance still requires APK validation.

Immersive VR defaults to FPS through its own persisted `nh3d-webxr-settings.fpsMode` preference (default true). The FPS toggle in the VR tab controls that preference; the in-session toggle uses it too. Leaving XR restores the flat client preference. The engine applies view transitions through its existing play-mode owner before sampling the game rig; terminal display stays top-down.

Startup/game handoff clears tracking readiness. The dungeon is withheld until a non-emulated, positive-height local-floor pose is available; calibration is repeated when the authoritative player position arrives. Both in-game Recenter world and native system reset refresh the browser local-floor reference and invalidate the complete rig, including scale. Reference-space reset events also request recalibration. `patch-floor-calibration.mjs` validates OpenXR stage transforms before publishing them, keeps the last good floor during tracking gaps, and notifies the game when a valid floor height changes.

Immersive FPS world scale replaces the old stereo-separation control. At 100%, `createTrackingToGame` uses the original calibrated metres-per-tile formula and unmodified headset eye poses. The 50–200% control scales the tracking-to-game mapping about the Quest-reported local-floor plane (tracking Y=0), so the dungeon floor stays at physical floor height on FPS entry, re-entry, recentering and live scale changes. Head height in game units changes with scale; controllers, held weapons and UI retain their physical size and alignment. Tabletop has a separate scale setting. Saved legacy `depth` values are discarded rather than converted; existing installations start FPS scale at 100%. The removed `StereoDepth` pass must not be reintroduced around individual draw passes.

The native compositor draws game lasers after HTML UI but before the tracked foreground redraw, so controller models and held weapons occlude the beams. `patchForeground` migrates both fresh and previously prepared checkouts to this order. Wired rendering already draws controller foreground after world lasers.

Position/travel instruction panels (`#position-dialog`) use dedicated pane 1. In immersive FPS their top edge follows the actual minimap's bottom edge with 6 cm padding (falling back to status if the minimap is hidden); in tabletop they sit just below status. Their source crop is removed from the full FPS HUD for both drawing and hit testing. The shared layout caps their width to 720 px or 54vw and keeps them separate from contextual popup anchoring. Background glance/Info probes do not display these instructions. Java and native snapshot validation accept up to nine panes to allow instructions alongside the complete HUD and modal/dropdown pair. This requires rebuilding the Quest APK; source tests and native syntax checks do not establish headset placement quality.

The immersive hotbar has a permanent round hamburger at its left edge. A fixed outer crop keeps its native pose stationary while the rounded inner shell expands/collapses in 180 ms. Hidden buttons are inert and excluded from hit rectangles; only the visible shell remains interactive, including native grip tests. Interactive glance starts collapsed and can be opened explicitly without changing the normal-play visibility preference. Synthetic contextual probes do not auto-hide the hotbar. Flat presentation retains the original layout and controls.
...
## Controller target previews and glance orbit

Immersive FPS LS activates headset-based highlighting at 5% radial deflection and confirms movement at 30%. The highlight continues following head look until the right laser changes tile offset relative to the player. Moving the player while the laser stays at the feet or at the same neighboring offset does not transfer ownership. Camera-follow interpolation is excluded from ownership comparisons; settled offsets are compared without a time-based cooldown. Laser targets otherwise update immediately. Accepted RT movement also selects laser highlighting, which stays active without further relative tile changes. A fresh LS crossing of the 5% threshold, an RS snap turn, or a 30-degree change in physical headset viewing direction restores headset ownership. RS turning clears the previous relative laser reference and establishes a new settled offset before laser movement can reclaim the highlight. LS movement and running always use camera-facing directions independently of the highlighted tile; RT activates the laser-targeted tile. The head-angle reference is captured on entering laser mode and is not reset by repeated RT moves; virtual snap turns and rectangular-world scaling do not count as physical head movement. Movement highlights allow the first missing cell adjacent (including diagonally) to visible passable terrain, but not a second cell, known walls, or map bounds overflow. Tabletop and glance can also show laser tile targeting. Selection columns and the highlight itself are excluded from controller world rays.

In glance, RS quarter-turns rotate the view around the current selection column while preserving world-up. Normal FPS still snap-turns around the viewer. The shared return animation also unwinds the glance orbit yaw before resuming ordinary FPS.
