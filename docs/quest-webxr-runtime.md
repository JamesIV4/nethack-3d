...
The Quest APK puts VR first in Options and selects it whenever Options opens. The flatten-items default is off; explicit saved choices are retained. Native immersive FPS draws actions first, then a depth-tested, alpha-tested loot mask replay, then the remaining HUD and dialogs. Controllers and held weapons retain the last foreground pass. Loot uses alpha=.75 and the fourth corner-tag value=.75 to distinguish the extra layer from older controller-only frames. The existing UI heartbeat carries a validated two-bit `X-NH3D-Loot-Hits` header; only the covered actions pane yields input to an opaque loot hit, while dialogs and existing UI captures retain priority. No CPU framebuffer readback or additional full-resolution render target is introduced. The device shader harness covers both eye rectangles/orientations, loot/action and modal ordering, controllers, MR alpha and legacy frames. Full headset interaction/performance still requires APK validation.
