...
## Temporary release sprite diagnostics

`QUEST_SPRITE_TRACE_ENABLED` in `src/quest/webxr/sprite-trace.ts` currently enables targeted diagnostics in the Quest APK, including release builds. Set it to `false` after the movement reproduction is resolved. The client records ordered player/under-player events, nearby tile-render decisions, billboard removal, and changed final sprite/proxy presentation. It batches at most five local requests per second with bounded queues and reports dropped records. `BundledGameServer` accepts bounded same-origin trace posts and writes the `NH3DSprites` Android log tag; this does not enable global JavaScript chatter, remote debugging, or Android's debuggable flag. Rebuild/reinstall the release APK before reproducing. Monitor with `adb logcat -v threadtime NH3DSprites:I '*:S'` and confirm a `trace-ready` record before moving.
...
The tabletop preserves its heading while the viewer moves. Only holding the table position handle enables facing updates; release freezes the resulting heading and recenter resets it. Actions/Extended panes open above their invoking hotbar button with a fixed bottom pivot and pitch chosen so the panel center faces the viewer. Pause and its submenus use ordinary centered placement, clearing the Actions anchor and its dragged offset. Immersive Extended uses 90px buttons, at most ten columns (954px including gaps), and immediate layout changes. These sizing and animation overrides are scoped to `nh3d-webxr-active` and do not affect flat presentation.
...
Position/travel instruction panels (`#position-dialog`) use dedicated pane 1 below the map, at the usual 1.4 m modal depth. Their source crop is removed from the full FPS HUD for both drawing and hit testing. The shared layout caps their width to 720 px or 54vw and keeps them separate from contextual popup anchoring. Java and native snapshot validation accept up to nine panes to allow instructions alongside the complete HUD and modal/dropdown pair. This requires rebuilding the Quest APK; source tests and native syntax checks do not establish headset placement quality.
