 ...
 * World alpha occupies [0,.5]; optional loot is .75; controllers are 1. Four reserved corner texels
...
export class NativeForeground {
  ...
  private readonly stamp = new THREE.ShaderMaterial({uniforms:{eyeSize:{value:new THREE.Vector2(1,1)},lastTag:{value:.625}},
    ...
    fragmentShader:'uniform float lastTag; varying vec2 uvLocal; void main(){ float i=floor(uvLocal.x*4.); float a=i<1.?0.625:i<2.?0.75:i<3.?0.875:lastTag; gl_FragColor=vec4(0.,0.,0.,a); }',
  ...
  finish(renderer:THREE.WebGLRenderer,camera:THREE.Camera,scene:THREE.Scene,hasLoot=false):void{
    ...
    this.stamp.uniforms.lastTag.value = hasLoot ? .75 : .625;
