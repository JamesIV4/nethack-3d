...
it("defaults to 150% resolution and bounds persisted values", () => {
  expect(normalizeXrSettings({})).toEqual({ resolution: 1.5, area: 1, scale: 1 });
  expect(normalizeXrSettings({ resolution: NaN, area: 99, scale: -1 })).toEqual({ resolution: 1.5, area: 2, scale: .5 });
});
