  ...
  return <div className="nh3d-xr-table-controls" data-xr-ui>
    <div className="nh3d-xr-table-icons">
      {(["area", "scale"] as const).map(key => <button type="button" key={key} className="nh3d-mobile-bottom-button"
        title={key === "area" ? t.webxr.visibleMapAreaLabel : t.webxr.worldScaleLabel} aria-label={key === "area" ? t.webxr.visibleMapAreaLabel : t.webxr.worldScaleLabel}
    ...
    {selected && <label>{selected === "area" ? t.webxr.visibleMapAreaLabel : t.webxr.worldScaleLabel}: {Math.round(settings[selected] * 100)}%
      <input aria-label={selected === "area" ? t.webxr.visibleMapAreaLabel : t.webxr.worldScaleLabel} type="range" min={selected === "area" ? 1 : .5} max="2" step="0.1"
