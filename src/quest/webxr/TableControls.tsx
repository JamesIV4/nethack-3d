  ...
  return <div className="nh3d-xr-table-controls" data-xr-ui>
    <div className="nh3d-xr-table-icons">
      {(["area", "scale"] as const).map(key => <button type="button" key={key} className="nh3d-mobile-bottom-button"
        title={key === "area" ? "Visible map area" : "World and UI scale"} aria-label={key === "area" ? "Visible map area" : "World and UI scale"}
    ...
    {selected && <label>{selected === "area" ? "Visible map area" : "World and UI scale"}: {Math.round(settings[selected] * 100)}%
      <input aria-label={selected === "area" ? "Visible map area" : "World and UI scale"} type="range" min={selected === "area" ? 1 : .5} max="2" step="0.1"
