...
// Tell TypeScript about globals provided by other scripts.  The
// `three.module.js` script defines a global called `THREE`, and
// `nethack.js` defines `nethackStart`.  We don't import these
// explicitly because they live on the global object when loaded via
// script tags.
declare const THREE: any;
declare const nethackStart: any;

/*
 * Interface used internally to track which NetHack window IDs are
 * associated with the map, messages, and status.  NetHack will call
 * `shim_create_nhwindow` with a window type string (e.g. "NHW_MAP").
 * We assign numeric identifiers that NetHack understands for later
 * callbacks.  Only one map window is supported in this simple port.
 */
interface WindowMap {
  [key: string]: number;
}
...
// separately.
const tileMeshes: Map<string, any> = new Map();
 ...
 */
function decodeGlyph(glyph: number, x: number, y: number): { ch: string; color: string } {
  try {
    const opts = (globalThis as any).nethackOptions;
 ...
 */
function updateTile(x: number, y: number, ch: string, color: string): void {
  const key = `${x},${y}`;
 ...
 */
async function uiCallback(name: string, ...args: any[]): Promise<any> {
  switch (name) {
...
async function startNethack(): Promise<void> {
