...
NetHack 5's first `getpos` can emit the informational "Tip: Farlooking or selecting a map location" menu. During a pending contextual glance or active Info probe, `RuntimeMenuCapture` suppresses only that tip's `info_menu` event, including when the first-use tip is delayed. Glance suppression ends on target consumption, cancellation, or an unrelated command. Menu classification, PICK_NONE completion, the queued target click and follow-up cancellation remain unchanged. Manual look commands and unrelated informational menus retain normal presentation. Engine-side synthetic position modes retain input ownership but publish `contextual-probe` as their UI origin so cursor help and hotbar auto-hide do not interrupt the context menu.
...
- Protocol v1 is owned by `src/runtime/protocol/`, `runtime-worker.ts`, and `WorkerRuntimeBridge.ts`. It adds session/command/observation identities around the existing owners. It must replay records through the established engine handler in order; it is not a second input queue or world model. `clear_scene` advances presentation generation without proving a level change. Runtime `u.uz` owns physical level generation.
- `RuntimeProtocolSession` observes the existing Promise waits. It buffers a prompt only until it knows whether that callback actually suspended, then attaches the request ID. Preserve Promise identity, broker FIFO/kind isolation, and separate received/consumed/closed states.
- `RuntimeRefreshRequests` deduplicates cell sets and distinguishes cached presentation from fresh completion. Requests stay pending while WASM helper re-entry is unsafe, cancel on generation change, and share one helper result across overlapping deferred sets.
...
Regression coverage lives in [input lifecycle tests](input/runtime-input-lifecycle.test.ts), [world refresh tests](world/runtime-world-refresh.test.ts), [protocol integration tests](runtime-protocol-integration.test.ts), [callback and menu tests](runtime-callback-state.test.ts), and [startup and snapshot tests](runtime-startup-snapshot.test.ts). They exercise assembled owners with controlled WASM boundaries, including pending request identity, cancellation, heap writes and hydration ordering.
