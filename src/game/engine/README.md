...
`src/quest/webxr/` owns in-game VR controls, controller command routing and the wired-only HTML capture pane. The standalone host uses pinned Wolvic and Gecko patches under `scripts/quest/webxr/` and `quest/webxr/`, including continued HTML painting during immersive VR. The checkout/build dependencies under `quest/runtime/` are ignored. Read [the runtime proof guide](../../../docs/quest-webxr-runtime.md) before changing rendering or compositor ownership. The legacy Meta Spatial exporter remains an earlier experiment.
