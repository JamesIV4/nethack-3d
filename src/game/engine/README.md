...
`src/quest/webxr/` owns in-game VR controls, laser/capture routing and the shared `HtmlUiPanel`. DOM hit testing passes transparent space through to the world. The standalone host uses pinned Wolvic and Gecko patches under `scripts/quest/webxr/` and `quest/webxr/`, including continued HTML painting during immersive VR and the shared pane pose. The checkout/build dependencies under `quest/runtime/` are ignored. Read [the runtime proof guide](../../../docs/quest-webxr-runtime.md) before changing rendering or compositor ownership. The legacy Meta Spatial exporter remains an earlier experiment.
