...
`rendering/held-weapon-flips.ts` resolves the shared flat FPS/VR sprite base: authored per-sprite flips from `held-weapon-flip-defaults.ts` override the built-in per-tile table, then tileset defaults provide the fallback. Flip defaults are looked up with the original pack tile ID through `TilesetAssets.resolveSourceTileIndexForRuntime`, including 3.6.7 atlases compiled into NetHack 5 order. Texture extraction continues using runtime tile IDs. Existing session edits in the flat FPS debug editor retain highest priority and remain keyed to runtime tiles. The standalone utility edits Flip X/Y/Diagonal for each sprite, displays a corrected flat thumbnail, and saves shared defaults independently of the VR pose library.
