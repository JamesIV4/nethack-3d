...
export const ko = {
  ...
  app: {
    actionCustomization: {
      savedImmediately: "변경 사항은 즉시 저장됩니다. 기본값 복원은 세 버튼 레이아웃을 모두 초기화합니다.",
      customize: "사용자 지정",
      mobileHotbar: "단축바 (모바일 / VR)",
      desktopHotbar: "단축바 (데스크톱)",
      menuActions: "메뉴 / 동작",
      buttonOrder: "버튼 순서",
      unavailableInGame: (label: string) => `${label} (이 게임에서 사용할 수 없음)`,
      moveEarlier: (label: string) => `${label}을(를) 앞으로 이동`,
      moveLater: (label: string) => `${label}을(를) 뒤로 이동`,
      remove: (label: string) => `${label} 제거`,
      removeButton: "제거",
      noButtonsAdded: "아직 추가된 버튼이 없습니다.",
      allCommands: "모든 명령",
      searchCommands: "명령 검색",
      searchPlaceholder: "이름으로 검색",
      add: (label: string) => `${label} 추가`,
      addButton: "추가",
      added: "추가됨",
      ...
      showHotbar: "단축바 표시",
      hideHotbar: "단축바 숨기기",
      customizeHotbar: "단축바 사용자 지정",
    },
    webxr: {
      groupLabel: "가상 현실",
      passthroughLabel: "VR 혼합 현실",
      passthroughDescription: "보드나 던전 주변에 방을 표시합니다. 다음에 VR에 들어갈 때 적용됩니다.",
      vrTabLabel: "VR",
      vrTabDescription: "헤드셋 표시 및 테이블 설정입니다.",
      buttonsTabLabel: "단축바",
      buttonsTabDescription: "단축바와 메뉴 / 동작 바로가기를 사용자 지정합니다.",
      enter: "VR 시작",
      exit: "VR 종료",
      opening: "VR 여는 중…",
      fpsWorldScaleLabel: "FPS 월드 배율",
      fpsWorldScaleDescription: "몰입형 1인칭 월드의 크기를 즉시 조정합니다. 100%는 원래 크기로 복원합니다. 컨트롤러, 손에 든 무기와 UI의 물리적 크기는 유지됩니다.",
      fpsScaleValue: (percent: number, isDefault: boolean) => `${percent}%${isDefault ? " (기본값)" : ""}`,
      fpsScaleAriaValue: (percent: number, isDefault: boolean) => `${percent}퍼센트${isDefault ? ", 기본값" : ""}`,
      defaultLabel: "기본값",
      resetFpsWorldScaleLabel: "FPS 월드 배율을 100%로 초기화",
      weaponGesturesLabel: "무기 제스처",
      weaponGesturesDescription: "1인칭 VR에서 손에 든 무기를 휘둘러 공격합니다. 다음 공격 전에 안정된 자세로 돌아오세요.",
      weaponSensitivityLabel: "무기 제스처 감도",
      weaponSensitivityDescription: "값이 높을수록 가벼운 휘두르기도 인식합니다.",
      tabletopVisibleAreaLabel: "테이블 표시 영역",
      tabletopVisibleAreaDescription: "타일 크기를 바꾸지 않고 테이블에 표시되는 지도 영역을 넓힙니다.",
      tabletopWorldScaleLabel: "테이블 월드 배율",
      tabletopWorldScaleDescription: "표시 지도 영역이나 UI 크기를 바꾸지 않고 테이블 월드 크기를 조정합니다.",
      renderResolutionLabel: "VR 렌더링 해상도",
      renderResolutionDescription: "해상도 변경은 다음 VR 진입 시 적용됩니다.",
      currentRenderSize: (size: string) => ` 현재 렌더링 크기: ${size}.`,
      instantMovementLabel: "즉시 이동",
      instantMovementDescription: "표준 1인칭 이동 애니메이션 대신 던전 타일 사이를 즉시 이동합니다.",
      recenterWorldLabel: "월드 중앙 맞추기",
      visibleMapAreaLabel: "표시 지도 영역",
      worldScaleLabel: "월드 배율",
      renderResolution: (width: number, height: number) => `눈당 ${width} × ${height}픽셀`,
      unsupportedHost: "이 호스트는 WebXR을 제공하지 않습니다. WebXR 런타임 빌드를 사용하세요.",
      noHeadset: "활성 XR 헤드셋 또는 런타임을 찾을 수 없습니다.",
      mixedRealityError: (detail: string) => `혼합 현실을 시작할 수 없습니다. 불투명 VR 세션을 테스트하려면 VR 혼합 현실을 끄세요. ${detail}`,
      hostQuitError: "VR 호스트가 앱을 종료하지 못했습니다.",
    },
    ...
    clientOptions: {
      config: {
        ...
        animatedMovement: {
          label: "움직임 애니메이션",
          description: "타일 및 3D 문자 모드에서 플레이어와 다른 생물이 칸 사이를 이동하는 모습을 애니메이션으로 표시합니다. 실제 터미널 모드는 항상 즉시 갱신됩니다.",
        },
        ...
        minimapColorMode: {
          label: "미니맵 색상",
          description: "NetHack 3D의 의미 기반 팔레트 또는 NetHack 터미널 색상을 선택합니다.",
          options: {
            nethack3d: "NetHack 3D",
            terminal: "터미널",
          },
