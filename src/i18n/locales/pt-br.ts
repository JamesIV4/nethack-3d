...
export const ptBrOverrides = {
  ...
  app: {
    actionCustomization: {
      savedImmediately: "As alterações são salvas imediatamente. Restaurar padrões redefine os três layouts de botões.",
      customize: "Personalizar",
      mobileHotbar: "Barra de ações (celular / VR)",
      desktopHotbar: "Barra de ações (desktop)",
      menuActions: "Menu / Ações",
      buttonOrder: "Ordem dos botões",
      unavailableInGame: (label: string) => `${label} (indisponível neste jogo)`,
      moveEarlier: (label: string) => `Mover ${label} para cima`,
      moveLater: (label: string) => `Mover ${label} para baixo`,
      remove: (label: string) => `Remover ${label}`,
      removeButton: "Remover",
      noButtonsAdded: "Nenhum botão foi adicionado ainda.",
      ...
      hotbar: "Barra de ações",
      showHotbar: "Mostrar barra de ações",
      hideHotbar: "Ocultar barra de ações",
      customizeHotbar: "Personalizar barra de ações",
    },
    webxr: {
      groupLabel: "Realidade virtual",
      passthroughLabel: "Realidade mista em VR",
      passthroughDescription: "Mostra seu ambiente ao redor do tabuleiro ou da masmorra. Entra em vigor na próxima vez que você entrar em VR.",
      vrTabLabel: "VR",
      vrTabDescription: "Apresentação do headset e configurações da mesa.",
      buttonsTabLabel: "Barra de ações",
      buttonsTabDescription: "Personalize barras de ações e atalhos de Menu / Ações.",
      enter: "Entrar em VR",
      exit: "Sair da VR",
      opening: "Abrindo VR…",
      fpsWorldScaleLabel: "Escala do mundo FPS",
      fpsWorldScaleDescription: "Redimensiona imediatamente o mundo imersivo em primeira pessoa. 100% restaura o tamanho original. Controles, armas empunhadas e UI mantêm seu tamanho físico.",
      fpsScaleValue: (percent: number, isDefault: boolean) => `${percent}%${isDefault ? " (padrão)" : ""}`,
      fpsScaleAriaValue: (percent: number, isDefault: boolean) => `${percent} por cento${isDefault ? ", padrão" : ""}`,
      defaultLabel: "Padrão",
      resetFpsWorldScaleLabel: "Redefinir a escala do mundo FPS para 100 por cento",
      weaponGesturesLabel: "Gestos com armas",
      weaponGesturesDescription: "Balance uma arma empunhada para atacar em VR em primeira pessoa. Recupere uma posição estável antes do próximo ataque.",
      weaponSensitivityLabel: "Sensibilidade dos gestos com armas",
      weaponSensitivityDescription: "Valores maiores aceitam golpes mais leves.",
      tabletopVisibleAreaLabel: "Área visível da mesa",
      tabletopVisibleAreaDescription: "Aumenta a área do mapa exibida na mesa sem alterar o tamanho dos tiles.",
      tabletopWorldScaleLabel: "Escala do mundo na mesa",
      tabletopWorldScaleDescription: "Redimensiona o mundo da mesa sem alterar a área visível do mapa nem o tamanho da UI.",
      renderResolutionLabel: "Resolução de renderização VR",
      renderResolutionDescription: "As alterações de resolução entram em vigor na próxima entrada na VR.",
      currentRenderSize: (size: string) => ` Tamanho de renderização atual: ${size}.`,
      instantMovementLabel: "Movimento instantâneo",
      instantMovementDescription: "Mova-se imediatamente entre os tiles da masmorra em vez de usar a animação de movimento padrão em primeira pessoa.",
      recenterWorldLabel: "Recentrar mundo",
      visibleMapAreaLabel: "Área visível do mapa",
      worldScaleLabel: "Escala do mundo",
      renderResolution: (width: number, height: number) => `${width} × ${height} pixels por olho`,
      unsupportedHost: "Este host não expõe WebXR. Use a compilação de runtime WebXR.",
      noHeadset: "Nenhum headset/runtime XR ativo foi encontrado.",
      mixedRealityError: (detail: string) => `Não foi possível entrar na realidade mista. Desative Realidade mista em VR para testar uma sessão de VR opaca. ${detail}`,
      hostQuitError: "O host VR não conseguiu sair do aplicativo.",
    },
    ...
    clientOptions: {
      config: {
        ...
        asciiColorMode: {
          label: "Cores ASCII",
          description: "Esquema de cores usado ao renderizar glifos ASCII em 3D. O NetHack 3D usa a paleta semântica 3D. Os modos Clássico e Terminal usam as cores exatas e o destaque do terminal do runtime ativo, com fundos cinza-escuro e preto, respectivamente.",
          options: {
        ...
        animatedMovement: {
          label: "Movimento animado",
          description: "Anima o jogador e outras criaturas entre células nos modos Tiles e ASCII 3D. O modo Terminal verdadeiro sempre atualiza imediatamente.",
        },
        ...
        minimapColorMode: {
          label: "Cores do minimapa",
          description: "Escolha a paleta semântica do NetHack 3D ou as cores do terminal do NetHack.",
          options: {
            nethack3d: "NetHack 3D",
            terminal: "Terminal",
          },
    ...
    saves: {
      ...
      loadExisting: "Carregar salvamento existente",
