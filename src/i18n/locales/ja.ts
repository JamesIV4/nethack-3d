...
export const jaOverrides: LocaleOverrides<TranslationDictionary> = {
  ...
  app: {
    actionCustomization: {
      savedImmediately: "変更はすぐに保存されます。「デフォルトに戻す」で3つすべてのボタン配置をリセットします。",
      customize: "カスタマイズ",
      mobileHotbar: "ホットバー（モバイル / VR）",
      desktopHotbar: "ホットバー（デスクトップ）",
      menuActions: "メニュー / アクション",
      buttonOrder: "ボタンの順序",
      unavailableInGame: (label: string) => `${label}（このゲームでは使用できません）`,
      moveEarlier: (label: string) => `${label}を前へ移動`,
      moveLater: (label: string) => `${label}を後ろへ移動`,
      remove: (label: string) => `${label}を削除`,
      removeButton: "削除",
      noButtonsAdded: "ボタンはまだ追加されていません。",
      allCommands: "すべてのコマンド",
      searchCommands: "コマンドを検索",
      searchPlaceholder: "名前で検索",
      add: (label: string) => `${label}を追加`,
      addButton: "追加",
      added: "追加済み",
      ...
      showHotbar: "ホットバーを表示",
      hideHotbar: "ホットバーを隠す",
      customizeHotbar: "ホットバーをカスタマイズ",
    },
    webxr: {
      groupLabel: "バーチャルリアリティ",
      passthroughLabel: "VRの複合現実",
      passthroughDescription: "ボードやダンジョンの周囲に部屋を表示します。次にVRへ入るときに適用されます。",
      vrTabLabel: "VR",
      vrTabDescription: "ヘッドセット表示とテーブル設定。",
      buttonsTabLabel: "ホットバー",
      buttonsTabDescription: "ホットバーとメニュー / アクションのショートカットをカスタマイズします。",
      enter: "VRに入る",
      exit: "VRを終了",
      opening: "VRを開始中…",
      fpsWorldScaleLabel: "FPSワールドスケール",
      fpsWorldScaleDescription: "一人称の没入型ワールドをすぐに拡大縮小します。100%で元のサイズに戻ります。コントローラー、手持ち武器、UIの物理サイズは変わりません。",
      fpsScaleValue: (percent: number, isDefault: boolean) => `${percent}%${isDefault ? "（デフォルト）" : ""}`,
      fpsScaleAriaValue: (percent: number, isDefault: boolean) => `${percent}パーセント${isDefault ? "、デフォルト" : ""}`,
      defaultLabel: "デフォルト",
      resetFpsWorldScaleLabel: "FPSワールドスケールを100%に戻す",
      weaponGesturesLabel: "武器ジェスチャー",
      weaponGesturesDescription: "一人称VRで手持ち武器を振って攻撃します。次の攻撃の前に静止した姿勢へ戻ってください。",
      weaponSensitivityLabel: "武器ジェスチャー感度",
      weaponSensitivityDescription: "値を高くすると、より小さな振りを受け付けます。",
      tabletopVisibleAreaLabel: "テーブル上の表示領域",
      tabletopVisibleAreaDescription: "タイルサイズを変えずに、テーブル上に表示するマップ領域を広げます。",
      tabletopWorldScaleLabel: "テーブルワールドスケール",
      tabletopWorldScaleDescription: "表示マップ領域やUIサイズを変えずに、テーブル上のワールドを拡大縮小します。",
      renderResolutionLabel: "VR描画解像度",
      renderResolutionDescription: "解像度の変更は次にVRへ入るときに適用されます。",
      currentRenderSize: (size: string) => ` 現在の描画サイズ: ${size}。`,
      instantMovementLabel: "即時移動",
      instantMovementDescription: "標準の一人称移動アニメーションを使わず、ダンジョンのタイル間をすぐに移動します。",
      recenterWorldLabel: "ワールドを中央に戻す",
      visibleMapAreaLabel: "表示マップ領域",
      worldScaleLabel: "ワールドスケール",
      renderResolution: (width: number, height: number) => `片目あたり${width} × ${height}ピクセル`,
      unsupportedHost: "このホストはWebXRを提供していません。WebXRランタイムビルドを使用してください。",
      noHeadset: "アクティブなXRヘッドセットまたはランタイムが見つかりません。",
      mixedRealityError: (detail: string) => `複合現実を開始できませんでした。透過なしのVRセッションを試すには、VRの複合現実をオフにしてください。${detail}`,
      hostQuitError: "VRホストがアプリを終了できませんでした。",
    },
    unknownTime: "不明な時刻",
    debugSession: {
