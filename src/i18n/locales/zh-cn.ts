...
export const zhCnOverrides = {
  ...
  app: {
    actionCustomization: {
      savedImmediately: "更改会立即保存。“恢复默认设置”会重置全部三个按钮布局。",
      customize: "自定义",
      mobileHotbar: "快捷栏（移动端 / VR）",
      desktopHotbar: "快捷栏（桌面端）",
      menuActions: "菜单 / 操作",
      buttonOrder: "按钮顺序",
      unavailableInGame: (label: string) => `${label}（在此游戏中不可用）`,
      moveEarlier: (label: string) => `将${label}上移`,
      moveLater: (label: string) => `将${label}下移`,
      remove: (label: string) => `移除${label}`,
      removeButton: "移除",
      noButtonsAdded: "尚未添加按钮。",
      allCommands: "所有命令",
      searchCommands: "搜索命令",
      ...
      showHotbar: "显示快捷栏",
      hideHotbar: "隐藏快捷栏",
      customizeHotbar: "自定义快捷栏",
    },
    webxr: {
      groupLabel: "虚拟现实",
      passthroughLabel: "VR 混合现实",
      passthroughDescription: "在棋盘或地城周围显示你的房间。下次进入 VR 时生效。",
      vrTabLabel: "VR",
      vrTabDescription: "头显显示和桌面设置。",
      buttonsTabLabel: "快捷栏",
      buttonsTabDescription: "自定义快捷栏以及菜单 / 操作快捷方式。",
      enter: "进入 VR",
      exit: "退出 VR",
      opening: "正在打开 VR…",
      fpsWorldScaleLabel: "FPS 世界缩放",
      fpsWorldScaleDescription: "立即调整沉浸式第一人称世界的大小。100% 会恢复原始大小。控制器、手持武器和界面保持其物理尺寸。",
      fpsScaleValue: (percent: number, isDefault: boolean) => `${percent}%${isDefault ? "（默认）" : ""}`,
      fpsScaleAriaValue: (percent: number, isDefault: boolean) => `${percent}%${isDefault ? "，默认" : ""}`,
      defaultLabel: "默认",
      resetFpsWorldScaleLabel: "将 FPS 世界缩放重置为 100%",
      weaponGesturesLabel: "武器手势",
      weaponGesturesDescription: "在第一人称 VR 中挥动手持武器进行攻击。下一次攻击前请回到安静姿态。",
      weaponSensitivityLabel: "武器手势灵敏度",
      weaponSensitivityDescription: "数值越高，越轻的挥动也会被接受。",
      tabletopVisibleAreaLabel: "桌面可见区域",
      tabletopVisibleAreaDescription: "在不改变图块大小的情况下，增大桌面上显示的地图区域。",
      tabletopWorldScaleLabel: "桌面世界缩放",
      tabletopWorldScaleDescription: "调整桌面世界大小，不改变可见地图区域或界面大小。",
      renderResolutionLabel: "VR 渲染分辨率",
      renderResolutionDescription: "分辨率更改会在下次进入 VR 时生效。",
      currentRenderSize: (size: string) => ` 当前渲染尺寸：${size}。`,
      instantMovementLabel: "即时移动",
      instantMovementDescription: "在地城图块之间立即移动，而不使用标准的第一人称移动动画。",
      recenterWorldLabel: "重新居中世界",
      visibleMapAreaLabel: "可见地图区域",
      worldScaleLabel: "世界缩放",
      renderResolution: (width: number, height: number) => `每只眼睛 ${width} × ${height} 像素`,
      unsupportedHost: "此主机不提供 WebXR。请使用 WebXR 运行时构建。",
      noHeadset: "未找到活动的 XR 头显/运行时。",
      mixedRealityError: (detail: string) => `无法进入混合现实。请关闭 VR 混合现实，以测试不透明的 VR 会话。${detail}`,
      hostQuitError: "VR 主机无法退出应用。",
    },
    unknownTime: "未知时间",
    ...
    clientOptions: {
      config: {
        ...
        asciiColorMode: {
          label: "ASCII 颜色",
          description: "用于渲染 3D ASCII 字形的配色方案。NetHack 3D 使用语义 3D 调色板；经典和终端模式使用当前运行时的准确颜色与终端高亮，背景分别为深灰色和黑色。",
          options: {
        ...
        animatedMovement: {
          label: "动画移动",
          description: "在瓦片和 3D ASCII 模式下，为玩家和其他生物在格子之间的移动添加动画。真正的终端模式始终立即更新。",
        },
        ...
        minimapColorMode: {
          label: "小地图颜色",
          description: "选择 NetHack 3D 语义调色板或 NetHack 的终端颜色。",
          options: {
            nethack3d: "NetHack 3D",
            terminal: "终端",
          },
