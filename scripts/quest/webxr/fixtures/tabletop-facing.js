...
const facing=new THREE.Vector3(0,-Math.hypot(head.x-center.x,head.y-center.y),head.z-center.z).normalize(),right=new THREE.Vector3().crossVectors(up0,facing).normalize(),up=new THREE.Vector3().crossVectors(facing,right).normalize();
...
for(const headPosition of [[-2,-4,3],[4,-4,3],[1,4,3],[1,-4,6]]){
head.fromArray(headPosition);facing.set(0,-Math.hypot(head.x-center.x,head.y-center.y),head.z-center.z).normalize();right.crossVectors(up0,facing).normalize();up.crossVectors(facing,right).normalize();mesh.position.copy(center).addScaledVector(up,.9);mesh.quaternion.setFromRotationMatrix(new THREE.Matrix4().makeBasis(right,up,facing));
for(const cameraScale of [1,1/.11]) for(const [yaw,roll] of [[0,0],[.2,.2],[-.3,-.3],[0,1.2]]){
 ...
 const camera=new THREE.ArrayCamera(cameras);camera.position.copy(head);camera.updateMatrixWorld();patch.prepare(scene,head,true);
 ...
 patch.disable();const old=render(true),b=render(false);patch.prepare(scene,head,true);const a=render(true);let diff=0,green=0,oldDiff=0;for(let i=0;i<a.length;i+=4){if(a[i+1]>100)green++;if(Math.max(Math.abs(a[i]-b[i]),Math.abs(a[i+1]-b[i+1]),Math.abs(a[i+2]-b[i+2]))>10)diff++;if(Math.max(Math.abs(old[i]-b[i]),Math.abs(old[i+1]-b[i+1]),Math.abs(old[i+2]-b[i+2]))>10)oldDiff++;}results.push({cameraScale,yaw,roll,diff,oldDiff,green});
}
}
renderer.dispose();target.dispose();if(results.some(r=>r.diff>12||r.green<20)||!results.some(r=>r.oldDiff>100))throw new Error(JSON.stringify(results));return {tabletopPitchOnly:true,caseCount:results.length,maxPixelDifference:Math.max(...results.map(r=>r.diff)),backsVisible:true};})();