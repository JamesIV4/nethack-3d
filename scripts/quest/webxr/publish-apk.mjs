...
if (!details.includes("versionName='0.3.5-native-pointer'")) throw new Error("Unexpected APK version.");
...
const inspected = unzipSync(bytes, { filter: (entry) => {
  ...
  return ["assets/nh3d-gecko-runtime.json", "lib/arm64-v8a/libxul.so", "res/raw/fxr_config.yaml"].includes(entry.name);
} });
...
const versionedOutput = path.join(path.dirname(output), "nethack3d-webxr-0.3.5-native-pointer-debug.apk");
