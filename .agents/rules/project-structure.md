...
- `src/quest/webxr/html-ui-panel.ts` and `dom-pointer.ts`: world-anchored pane and live DOM picking/capture for native and wired VR. The native host consumes the same pose through the bounded `/__xr/native-pointer` endpoint.
