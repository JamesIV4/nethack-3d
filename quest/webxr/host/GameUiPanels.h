 ...
 public:
  ...
  void Update(const WidgetPtr& source, const std::vector<float>& state,
    ...
    for (size_t i = 0; i < count; ++i) {
      ...
      const float width = 3.0f * (crop[2] - crop[0]);
      const float height = 3.0f * float(textureHeight) / textureWidth * (crop[3] - crop[1]);
      ...
      p.id = int(state[at]);
      if (p.id == 4) p.pose = center;
      else {
        vrb::Vector offset(0, 0.012f, 0);
        if (p.id == 0) offset.z() = -0.99f - height / 2;
        if (p.id == 3) offset.z() = 0.99f + height / 2;
        if (p.id == 1) offset.x() = -1.44f - width / 2;
        if (p.id == 2) offset.x() = 1.44f + width / 2;
