    ...
    private static void serve(AssetManager assets, Socket client) {
        try (Socket socket = client) {
            ...
            if (parts[0].equals("POST")) {
                ...
                if (count < 0 || count > 128 || panels < 1 || panels > 7 || values.length() != 29 + count * 4 + panels * 5) throw new IOException("Invalid table UI snapshot");
                ...
                for (int i = 29 + count * 4; i < pose.length; i += 5) {
                    if (pose[i] < 0 || pose[i] > 10 || pose[i] != (int)pose[i]) throw new IOException("Invalid pane ID");
